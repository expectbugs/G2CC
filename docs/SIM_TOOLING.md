...
> **⚠ 2026-06-29 — the sim moved, and is now pinned in `sdk-demo`.** `g2code`/`g2aria` (its old home)
> were archived to `/home/user/g2-old-backup-2026-06-24.tar.gz` and deleted. The sim is now a **`sdk-demo`
> devDependency**, so `cd /home/user/G2CC/sdk-demo && npm i` installs it to
> `sdk-demo/node_modules/@evenrealities/evenhub-simulator/bin/index.js` (the launch block below uses that
> path). The system setup (egl-gbm, nvidia-drm, `gtkwl_stub.so`) is intact.
...
GDK_BACKEND=x11 LD_PRELOAD=/home/user/G2CC/scripts/simtools/gtkwl_stub.so DISPLAY=:0.0 \
  nohup node /home/user/G2CC/sdk-demo/node_modules/@evenrealities/evenhub-simulator/bin/index.js \
