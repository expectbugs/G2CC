...
| `channel-router.ts` | NEW (Phase 7) | New | Per-client server-side ack tracker — assigns messageIds, awaits BleAckMsg, falls to `unverified` after BLE_ACK_WINDOW_MS. `setTimeout` inside is a STATUS-window guard, NOT an I/O timeout — operation continues regardless (spec §10 / FORBIDDEN_PATTERN_AUDIT §A). `supersede()` clears prior entries loudly. |
| `endpoints.ts` | NEW (uses setup-page's `classifyInterface`) | New | `/endpoints` JSON for client refetch on each successful auth (Phase 3A robustness; g2aria-style pull pattern). Same priority sort as `setup-page.ts`. Token-gated via `Authorization: Bearer`. |
...
| `pipeline/parakeet_cli.py` (Phase 8) | NEW (subprocess shim) | New | Tiny stdin→stdout shim: reads a WAV path from argv, calls `get_engine().transcribe(...)`, prints text. Invoked by `server/src/stt.ts` via `execFile` so Node never imports NeMo. Loud-fail on any input contract violation. |
