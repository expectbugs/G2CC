...
export async function buildPageMap(bookPath: string): Promise<PageMap> {
  const fingerprint = fingerprintOf(bookPath)
  ...
  const counts = data.chapters.map((c) => paginateText(c.text).length)
